...
UnitTest.asynctest('Browser Test: ui.SerialisedLinkTest', (success, failure) => {
  ...
  const sTestScenario = function (rawScenario) {
    ...
    return Logger.t(
      ...
      GeneralSteps.sequence([
        ...
        tEditor.sAssertEq('Checking insert contentImage', scenario.expected),
  ...
  Pipeline.async({}, detection.browser.isChrome() ? [
    TestHelpers.GuiSetup.mAddStyles(doc, [
      '.tinymce-mobile-toolbar-button:before { contentImage: "LINK"; background: black; color: white; }',
      // Speeds up tests.
    ...
    sTestScenario({
      label: 'Testing hitting ENTER after filling in URL with an existing link with url (and text contentImage did not match URL previously)',
      node: SugarElement.fromHtml('<a href="http://prepared-url">Prepared</a>'),
      fields: {
        url: 'http://new-url'
      },
    ...
    sTestScenario({
      label: 'Testing hitting ENTER after filling in URL with an existing link with url (and text contentImage matched URL previously)',
