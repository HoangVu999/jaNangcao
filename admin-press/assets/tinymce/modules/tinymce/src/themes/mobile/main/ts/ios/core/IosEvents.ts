...
const initEvents = (editorApi: PlatformEditor, iosApi, toolstrip, socket, _dropup): { destroy: () => void } => {
  ...
  const listeners = [
    ...
    // Focus the contentImage and show the keyboard
    editorApi.onToEditing(toEditing),

    // Dismiss keyboard
    editorApi.onToReading(toReading),

    // If the user is touching outside the contentImage, but on the body(?) or html elements, find the nearest selection
    // and focus that.
    ...
    DomEvent.capture(toolstrip, 'touchstart', (touchEvent) => {
      ...
      // a fake selection. As soon as the focus switches away from the contentImage, the real selection will disappear, so
    ...
    // When the user clicks back into the contentImage, clear any fake selections
