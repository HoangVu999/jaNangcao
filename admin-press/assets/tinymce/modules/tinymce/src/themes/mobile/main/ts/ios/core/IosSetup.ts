...
const register = function (toolstrip, socket, container, outerWindow, structure, cWin) {
  ...
  const onScroll = DomEvent.bind(SugarElement.fromDom(outerWindow), 'scroll', function () {
    ...
    of the contentImage, but would break the code that tried to position the text in the viewable area
    ...
    the socket so that the bottom of the contentImage can be scrolled into the viewable greenzone. If it doesn't
    ...
    However, if the padding is too large, then the contentImage can be 'effectively' scrolled off the screen
    (the iframe anyway), and the user can get lost about where they are. Our short-term fix is just to
    make the padding at the end the height - the greenzone height so that contentImage should always be
