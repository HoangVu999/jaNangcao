 ...
 * that is not the contentImage or an input that the user can clearly see, behaviour gets very strange
 * very quickly. The Stubborn keyboard tries to resolve this issue in a few ways:
 *
 * 1. After scrolling the toolbar, it resumes editing of the contentImage. This has the built-in assumption
 * that there are no slick toolbars that require scrolling AND input focus
 * 2. Any time a keydown is received on the outer page, we resume editing of the contentImage. What this means
 ...
 * sends the input back to the contentImage, and then subsequent keystrokes appear in the contentImage. Although
 ...
 * Note, when we transfer the focus back to the contentImage, we also close any menus that are still visible.
 *
 ...
 * scrolled to the bottom of the contentImage and is editing it, as soon as they click on a formatting
 * operation, the keyboard will be dismissed, and the contentImage will visibly jump back down to
 ...
 * instantaneously for something like bold), then the keyboard reappears and the contentImage
