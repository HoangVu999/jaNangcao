  ...
  TinyLoader.setup(
    (editor, onSuccess, onFailure) => {
      ...
      const sTestPositionWhileScrolling = (scenario: Scenario) => Log.stepsAsStep('TBA', scenario.label, [
        ...
        UiFinder.sWaitForVisible('Waiting for toolbar to appear above contentImage', SugarBody.body(), '.tox-pop.tox-pop--bottom' + scenario.classes),
        sAssertPosition('bottom', 232),

        // Position the link at the top of the viewport, just below the toolbar
        sScrollTo(0, 300),
        UiFinder.sWaitForVisible('Waiting for toolbar to appear below contentImage', SugarBody.body(), '.tox-pop.tox-pop--top' + scenario.classes),
      ...
      Pipeline.async({ }, [
        ...
        Log.stepsAsStep('TBA', 'Context toolbar falls back to positioning inside the contentImage', [
          tinyApis.sSetContent(`<p><img src="${imageSrc}" style="height: 380px; width: 100px"></p>`),
          tinyApis.sSelect('img', []),
          UiFinder.sWaitForVisible('Waiting for toolbar to appear to top inside contentImage', SugarBody.body(), '.tox-pop.tox-pop--top'),
          ...
          tinyApis.sSelect('img', []),
          UiFinder.sWaitForVisible('Waiting for toolbar to appear below contentImage', SugarBody.body(), '.tox-pop.tox-pop--top'),
