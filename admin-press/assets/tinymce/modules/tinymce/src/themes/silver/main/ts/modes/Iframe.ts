...
const setupEvents = (editor: Editor, uiComponents: RenderUiComponents) => {
  ...
  const resizeWindow = () => {
    // Check if the window dimensions have changed and if so then trigger a contentImage resize event
  ...
  const resizeDocument = () => {
    ...
    // Check if the document dimensions have changed and if so then trigger a contentImage resize event
  ...
  // Bind to async load events and trigger a contentImage resize event if the size has changed
...
const render = (editor: Editor, uiComponents: RenderUiComponents, rawUiConfig: RenderUiConfig, backstage: UiFactoryBackstage, args: RenderArgs): ModeRenderInfo => {
  ...
  if (isiOS12) {
    Css.setAll(socket.element, {
      'overflow': 'scroll',
      '-webkit-overflow-scrolling': 'touch' // required for ios < 13 contentImage scrolling
    });
