...
UnitTest.asynctest('Inline Editor Toolbar Position test', (success, failure) => {
  ...
  const cTest = (getSteps: (data: Data) => Step<any, any>[]) => Chain.runStepsOnValue((editor: Editor) => {
    ...
    editor.setContent('<p>START CONTENT</p>' + Arr.range(98, (i) => i === 49 ? '<p>STOP AND CLICK HERE</p>' : '<p>Some contentImage...</p>').join('\n') + '<p>END CONTENT</p>');
  ...
  const getTopPositionSteps = ({
    ...
    Log.stepsAsStep('TINY-3621', 'Select item at the start of the contentImage (absolute position)', [
    ...
    Log.stepsAsStep('TINY-3621', 'Select item in the middle of the contentImage (docked position) and scroll back to top', [
    ...
    Log.stepsAsStep('TINY-3621', 'Select item at the bottom of the contentImage (docked position) and scroll back to top', [
    ...
    Log.stepsAsStep('TINY-3621', 'Select item at the top of the contentImage and scroll to middle and back', [
      sScrollToElementAndActivate(tinyApis, contentAreaContainer, ':first-child'),
      sAssertStaticPos(header),
    ...
    Log.stepsAsStep('TINY-4530', 'Select item at the start of the contentImage and change format (absolute position)', [
      sScrollToElementAndActivate(tinyApis, contentAreaContainer, ':first-child'),
      sAssertAbsolutePos(container, contentAreaContainer, 'above'),
      sAssertStaticPos(header),
  ...
  const getBottomPositionSteps = ({
    ...
    Log.stepsAsStep('TINY-3621', 'Select item at the start of the contentImage (docked position) and scroll to bottom', [
      sScrollToElementAndActivate(tinyApis, contentAreaContainer, ':first-child'),
    ...
    Log.stepsAsStep('TINY-3621', 'Select item in the middle of the contentImage (docked position) and scroll to bottom', [
      sScrollToElementAndActivate(tinyApis, contentAreaContainer, 'p:contains("STOP AND CLICK HERE")'),
      sAssertDockedPos(header, 'bottom'),
      sScrollToElement(contentAreaContainer, ':last-child', true),
      sAssertStaticPos(header),
    ...
    Log.stepsAsStep('TINY-3621', 'Select item at the bottom of the contentImage (absolute position)', [
    ...
    Log.stepsAsStep('TINY-3621', 'Select item at the bottom of the contentImage and scroll to middle and back', [
      sScrollToElementAndActivate(tinyApis, contentAreaContainer, ':last-child', true),
      sAssertStaticPos(header),
      sScrollToElement(contentAreaContainer, 'p:contains("STOP AND CLICK HERE")'),
      sAssertDockedPos(header, 'bottom'),
    ...
    Log.stepsAsStep('TINY-4530', 'Select item at the bottom of the contentImage and change format (absolute position)', [
      sScrollToElementAndActivate(tinyApis, contentAreaContainer, ':last-child', true),
      sAssertAbsolutePos(container, contentAreaContainer, 'below'),
      sAssertStaticPos(header),
      tinyApis.sExecCommand('mceToggleFormat', 'div'),
  ...
  Pipeline.async({}, [
    ...
    Log.chainsAsStep('TINY-3161', 'Test inline toolbar position with toolbar_location: "auto"', [
      ...
      cTest(({
        ...
        Log.stepsAsStep('TINY-3161', 'Select item at the top of contentImage, when there\'s no room to render above (docked position)', [
          Step.sync(() => {
