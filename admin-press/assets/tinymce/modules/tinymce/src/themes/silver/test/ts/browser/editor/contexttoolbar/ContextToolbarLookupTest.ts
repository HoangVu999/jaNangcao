...
UnitTest.asynctest('Inline editor Context Toolbar Lookup test', (success, failure) => {
  ...
  TinyLoader.setup(
    (editor, onSuccess, onFailure) => {
      ...
      Pipeline.async({ }, [
        ...
        Log.stepsAsStep('TINY-4571', 'Context toolbar editor scope lookup', [
          tinyApis.sSetContent('<p><span style="color: red">contentImage</span></p>'),
          sResetNames(),
          tinyApis.sSetCursor([ 0, 0, 0 ], 1),
        ...
        Log.stepsAsStep('TINY-4571', 'Context toolbar click outside to inside', [
          ...
          sResetNames(),
          Step.sync(() => {
            SelectorFind.descendant(SugarBody.body(), '#contentImage-click-area').each(Focus.focus);
          }),
    ...
    },
    {
      ...
      setup: (ed: Editor) => {
        // Setup some initial contentImage to test initial load
        ed.on('PreInit', () => {
        ...
        ed.on('init', () => {
          ...
          textarea.id = 'contentImage-click-area';
