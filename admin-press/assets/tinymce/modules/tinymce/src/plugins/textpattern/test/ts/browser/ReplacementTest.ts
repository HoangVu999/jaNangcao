  ...
  TinyLoader.setupLight((editor, onSuccess, onFailure) => {
    ...
    const sAssertContentAndCursor = (beforeType: string, afterType?: string) => {
      ...
      return GeneralSteps.sequence([
        Step.label('Check contentImage', tinyApis.sAssertContent(beforeType)),
    ...
    const steps = Utils.withTeardown([
      ...
      Logger.t('Apply text replacement pattern on space with contentImage before', GeneralSteps.sequence([
        Utils.sSetContentAndPressSpace(tinyApis, tinyActions, 'Xbrb'),
        sAssertContentAndCursor('<p>Xbe right back&nbsp;|</p>')
      ])),
      Logger.t('Apply text replacement pattern on space with contentImage after', GeneralSteps.sequence([
        Utils.sSetContentAndPressSpace(tinyApis, tinyActions, 'brbX', 3),
        sAssertContentAndCursor('<p>be right back |X</p>')
      ])),
      Logger.t('Do not replace on pattern with contentImage after when cursor is in the wrong position', GeneralSteps.sequence([
      ...
      Logger.t('Apply text replacement pattern on enter with contentImage before', GeneralSteps.sequence([
        Utils.sSetContentAndPressEnter(tinyApis, tinyActions, 'Xbrb'),
        sAssertContentAndCursor('<p>Xbe right back</p><p>&nbsp;|</p>')
      ])),
      Logger.t('Apply text replacement pattern on enter with contentImage after', GeneralSteps.sequence([
