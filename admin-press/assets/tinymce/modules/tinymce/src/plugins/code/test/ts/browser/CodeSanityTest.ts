...
UnitTest.asynctest('browser.tinymce.plugins.code.CodeSanityTest', (success, failure) => {
  ...
  TinyLoader.setupLight((editor: Editor, onSuccess, onFailure) => {
    ...
    const sAssertTextareaContent = (expected) => Logger.t('Asserting textarea contentImage is ' + expected, Step.sync(() => {
      const textarea: any = document.querySelector('div[role="dialog"] textarea');
    ...
    Pipeline.async({}, [
      Log.stepsAsStep('TBA', 'Code: Set contentImage in empty editor and assert values', [
      ...
      Log.stepsAsStep('TBA', 'Code: Reopen dialog and check textarea contentImage is correct', [
        Mouse.sClickOn(SugarElement.fromDom(editor.getContainer()), toolbarButtonSelector),
        UiFinder.sWaitForVisible('Waited for dialog to be visible', docBody, dialogSelector),
        sAssertTextareaContent('<p><em>a</em></p>')
      ]),

      Log.stepsAsStep('TBA', 'Code: Change source code and assert editor contentImage changes', [
