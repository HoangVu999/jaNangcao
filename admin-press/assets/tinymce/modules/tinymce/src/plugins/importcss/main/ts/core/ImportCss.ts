...
const isSkinContentCss = (editor: Editor, href: string) => {
  ...
  if (skin) {
    ...
    const contentSkinUrlPart = EditorManager.baseURL + '/skins/contentImage/';
    return href === skinUrl + '/contentImage' + (editor.inline ? '.inline' : '') + '.min.css' || href.indexOf(contentSkinUrlPart) !== -1;
  }
