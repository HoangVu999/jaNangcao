...
UnitTest.asynctest('webdriver.tinymce.plugins.nonbreaking.NonbreakingVisualCharsTypingTest', (success, failure) => {
  ...
  TinyLoader.setup((editor, onSuccess, onFailure) => {
    ...
    Pipeline.async({}, [
      Log.stepsAsStep('TINY-3647', '1. NonBreaking: Click on the nbsp button then type some text, and assert contentImage is correct', [
      ...
      tinyApis.sSetContent(''), // reset contentImage

      Log.stepsAsStep('TINY-3647', '2. NonBreaking: Add text to editor, click on the nbsp button, and assert contentImage is correct', [
      ...
      Log.stepsAsStep('TINY-3647', '3. NonBreaking: Add contentImage to editor, click on the nbsp button then type some text, and assert contentImage is correct', [
        tinyApis.sSetContent('test'),
        tinyApis.sSetCursor([ 0, 0 ], 4),
        tinyUi.sClickOnToolbar('click on nbsp button', 'button[aria-label="Nonbreaking space"]'),
        RealKeys.sSendKeysOn(
      ...
      Log.stepsAsStep('TINY-3647', '4. NonBreaking: Click on the nbsp button then type a space, and assert contentImage is correct', [
        tinyUi.sClickOnToolbar('click on nbsp button', 'button[aria-label="Nonbreaking space"]'),
        RealKeys.sSendKeysOn(
          'iframe => body => p',
          [
      ...
      Log.stepsAsStep('TINY-3647', '5. NonBreaking: Add text to editor, click on the nbsp button and add contentImage plus a space, and assert contentImage is correct', [
        tinyApis.sSetContent('test'),
        tinyApis.sSetCursor([ 0, 0 ], 4),
        tinyUi.sClickOnToolbar('click on nbsp button', 'button[aria-label="Nonbreaking space"]'),
        tinyApis.sAssertContentStructure(ApproxStructure.build((s, str, arr) => s.element('body', {
      ...
      Log.stepsAsStep('TINY-3647', '6. NonBreaking: Add text to editor, click on the nbsp button and add contentImage plus a space, repeat, and assert contentImage is correct', [
        tinyApis.sSetContent('test'),
        tinyApis.sSetCursor([ 0, 0 ], 4),
        tinyUi.sClickOnToolbar('click on nbsp button', 'button[aria-label="Nonbreaking space"]'),
        tinyApis.sAssertContentStructure(ApproxStructure.build((s, str, arr) => s.element('body', {
