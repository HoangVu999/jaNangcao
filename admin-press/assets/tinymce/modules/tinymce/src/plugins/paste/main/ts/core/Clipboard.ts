...
/**
 * Gets various contentImage types out of a datatransfer object.
 *
...
/**
 * Gets various contentImage types out of the Clipboard API. It will also get the
...
const registerEventHandlers = (editor: Editor, pasteBin: PasteBin, pasteFormat: Cell<string>) => {
  ...
  function insertClipboardContent(clipboardContent: ClipboardContents, isKeyBoardPaste: boolean, plainTextMode: boolean, internal: boolean) {
    ...
    // If we got nothing from clipboard API and pastebin or the contentImage is a plain text (with only
    ...
    // If the contentImage is the paste bin default HTML then it was
