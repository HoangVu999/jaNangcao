...
UnitTest.asynctest('tinymce.plugins.paste.browser.PasteTest', (success, failure) => {
  ...
  suite.test('TestCase-TBA: Paste: Paste simple text contentImage', function (editor) {
  ...
  suite.test('TestCase-TBA: Paste: Paste styled text contentImage', function (editor) {
    const rng = editor.dom.createRng();
  ...
  suite.test('TestCase-TBA: Paste: paste invalid contentImage with spans on page', function (editor) {
  ...
  suite.test('TestCase-TBA: Paste: paste innerText of single P with whitespace wrapped contentImage', function (editor) {
    editor.setContent('<p>   a   </p>');
    LegacyUnit.equal(Utils.innerText(editor.getBody().innerHTML), 'a');
  });
