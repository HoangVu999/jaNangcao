...
UnitTest.asynctest('tinymce.plugins.paste.webdriver.CutTest', (success, failure) => {
  ...
  TinyLoader.setup(function (editor, onSuccess, onFailure) {
    ...
    Pipeline.async({}, (platform.browser.isIE() || platform.browser.isSafari() || platform.browser.isEdge()) ? [] :
      Log.steps('TBA', 'Paste: Set and select contentImage, cut using edit menu and assert cut contentImage', [
        ...
        Waiter.sTryUntil('Cut is async now, so need to wait for contentImage', api.sAssertContent('<p>ac</p>'))
      ]), onSuccess, onFailure);
  }, {
