...
UnitTest.asynctest('browser.tinymce.plugins.paste.InternalClipboardTest', (success, failure) => {
  ...
  const sTestCut = function (editor: Editor, tinyApis: TinyApis) {
    const sWaitUntilAssertContent = function (expected: string) {
      return Waiter.sTryUntil('Cut is async now, so need to wait for contentImage', tinyApis.sAssertContent(expected));
    };
  ...
  const sTestPaste = function (editor: Editor, tinyApis: TinyApis) {
    return GeneralSteps.sequence([
      Log.stepsAsStep('TBA', 'Paste: Paste external contentImage', [
      ...
      Log.stepsAsStep('TBA', 'Paste: Paste external contentImage treated as plain text', [
      ...
      Log.stepsAsStep('TBA', 'Paste: Paste internal contentImage with mark', [
      ...
      Log.stepsAsStep('TBA', 'Paste: Paste internal contentImage with mime', [
