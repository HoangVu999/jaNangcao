...
UnitTest.asynctest('browser.tinymce.plugins.wordcount.PluginTest', (success, failure) => {
  ...
  const sReset = function (tinyApis: TinyApis) {
    return Logger.t('Reset contentImage', GeneralSteps.sequence([
  ...
  const sTestSetContent = function (tinyApis: TinyApis) {
    return GeneralSteps.sequence(Log.steps('TBA', 'WordCount: Set test contentImage and assert word count', [
      sReset(tinyApis),
