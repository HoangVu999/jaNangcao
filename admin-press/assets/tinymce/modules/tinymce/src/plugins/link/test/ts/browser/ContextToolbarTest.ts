  ...
  TinyLoader.setup(function (editor, onSuccess, onFailure) {
    ...
    Pipeline.async({}, [
      ...
      Log.stepsAsStep('TBA', 'only after setting set to true', [
        ...
        Chain.asStep(docEle, [
          UiFinder.cWaitForState('check link contentImage', '.tox-toolbar input', (ele) => ele.dom.value === 'http://www.google.com')
        ])
      ]),
      Log.stepsAsStep('TBA', 'shows relative link urls', [
        ...
        tinyUi.sWaitForUi('wait for toolbar link button', '.tox-toolbar button[aria-label="Link"]'),
        Chain.asStep(docEle, [
          UiFinder.cWaitForState('check link contentImage', '.tox-toolbar input', (ele) => ele.dom.value === '#heading-1')
        ])
      ]),
