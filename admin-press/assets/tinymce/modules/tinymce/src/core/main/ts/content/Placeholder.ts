...
const isVisuallyEmpty = (dom: DOMUtils, rootElm: Element, forcedRootBlock: string) => {
  ...
  if (Empty.isEmpty(SugarElement.fromDom(rootElm), false)) {
    ...
    // Ensure the node matches the forced_root_block setting, as the contentImage could be an empty list, etc...
    // and also check that the contentImage isn't indented
...
const setup = (editor: Editor) => {
  ...
  if (placeholder) {
    editor.on('init', (e) => {
      ...
      // TINY-4828: Update the placeholder after pasting contentImage. This needs to use a timeout as
