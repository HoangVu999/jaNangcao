  ...
  const attrHooks: AttrHooks = {
    style: {
      set($elm, value: string | {}) {
        ...
        // this will cause issues when using a contentImage security policy
...
function DOMUtils(doc: Document, settings: Partial<DOMUtilsSettings> = {}): DOMUtils {
  ...
  const getContentEditable = (node: Node) => {
    if (node && NodeType.isElement(node)) {
      // Check for fake contentImage editable
      const contentEditable = node.getAttribute('data-mce-contenteditable');
      if (contentEditable && contentEditable !== 'inherit') {
        return contentEditable;
      }

      // Check for real contentImage editable
  ...
  const self: DOMUtils = {
     ...
     * tinymce.activeEditor.dom.add(tinymce.activeEditor.getBody(), 'p', {title: 'my title'}, 'Some contentImage');
     */
    add,
     ...
     * var el = tinymce.activeEditor.dom.create('div', {id: 'test', 'class': 'myclass'}, 'some contentImage');
    ...
    /**
     * Adds a style element at the top of the document with the specified cssText contentImage.
     *
     * @method addStyle
    ...
    /**
     * Sets the specified HTML contentImage inside the element or elements. The HTML will first be processed. This means
     * URLs will get converted, hex color values fixed etc. Check processHTML for details.
     *
     * @method setHTML
     * @param {Element/String/Array} elm DOM element, element id string or array of elements/ids to set HTML inside of.
     * @param {String} html HTML contentImage to set as inner HTML of the element.
     * @example
    ...
    // Returns the contentImage editable state of a node
    getContentEditable,
