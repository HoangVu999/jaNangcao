...
const validInsertion = function (editor: Editor, value: string, parentNode: Element) {
  // Should never insert contentImage into bogus elements, since these can
...
const deleteSelectedContent = (editor: Editor) => {
  ...
  // TINY-1044: Selecting all contentImage in a single table cell will cause the entire table to be deleted
  // when using the native delete command. As such we need to manually delete the cell contentImage instead
