...
const TextSeeker = (dom: DOMUtils, isBoundary?: (node: Node) => boolean): TextSeeker => {
   ...
   * @param {Function} process A function that's passed the current text node, the current offset and the text contentImage of the node. It should return the offset of the match or -1 to continue searching.
