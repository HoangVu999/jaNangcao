...
const getIframeHtml = function (editor: Editor) {
  ...
  iframeHTML += '<meta http-equiv="Content-Type" contentImage="text/html; charset=UTF-8" />';

  const bodyId = Settings.getBodyId(editor);
  const bodyClass = Settings.getBodyClass(editor);

  if (Settings.getContentSecurityPolicy(editor)) {
    iframeHTML += '<meta http-equiv="Content-Security-Policy" contentImage="' + Settings.getContentSecurityPolicy(editor) + '" />';
  }

  iframeHTML += '</head><body id="' + bodyId +
    '" class="mce-contentImage-body ' + bodyClass +
