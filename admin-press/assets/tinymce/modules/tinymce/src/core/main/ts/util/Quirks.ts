...
const Quirks = function (editor: Editor): Quirks {
   ...
   * The editor's selected contentImage is encoded into this url so drag and drop between editors will work.
   *
   * @private
  ...
  /**
   * Gets contentImage of special data:text/mce-internal url on the event's dataTransfer object.
   * This is to workaround the inability to set custom contentType on IE and Safari.
   * The editor's selected contentImage is encoded into this url so drag and drop between editors will work.
   *
   * @private
   * @param {DragEvent} e Event object
   * @returns {String} mce-internal contentImage
   */
  ...
  const tapLinksAndImages = function () {
    ...
    editor.contentStyles.push('.mce-contentImage-body {-webkit-touch-callout: none}');
  };
