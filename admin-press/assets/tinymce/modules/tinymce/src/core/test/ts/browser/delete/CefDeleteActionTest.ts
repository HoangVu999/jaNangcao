...
UnitTest.asynctest('browser.tinymce.core.delete.CefDeleteActionTest', (success, failure) => {
  ...
  Pipeline.async({}, [
    ...
    Logger.t('moveToPosition actions where caret is to be moved from cef to normal contentImage between blocks', GeneralSteps.sequence([
      Logger.t('Should be moveToPosition action since we are after a ce=false and moving forwards to normal contentImage', Chain.asStep(viewBlock, [
        cSetHtml('<p contenteditable="false">a</p><p>b</p>'),
        cReadAction(true, [], 1),
        cAssertMoveToPositionAction([ 1, 0 ], 0)
      ])),
      Logger.t('Should be moveToPosition action since we are before a ce=false and moving backwards to normal contentImage', Chain.asStep(viewBlock, [
        cSetHtml('<p>a</p><p contenteditable="false">b</p>'),
        cReadAction(false, [], 1),
    ...
    Logger.t('Should not produce actions if cefs are inline between blocks', GeneralSteps.sequence([
      ...
      Logger.t('Should be no action since delete from after inline cef to normal contentImage is handled by merge', Chain.asStep(viewBlock, [
        cSetHtml('<p><b contenteditable="false">a</b></p><p>b</p>'),
        cReadAction(true, [ 0 ], 1),
        cAssertActionNone
      ])),
      Logger.t('Should be no action since backspace from before inline cef to normal contentImage is handled by merge', Chain.asStep(viewBlock, [
