...
UnitTest.asynctest('browser.tinymce.core.EditorTest', (success, failure) => {
  ...
  const sWebkidSerializationRangeBug = (editor: Editor) => Log.step('TBA', 'WebKit Serialization range bug', Step.sync(() => {
    if (Env.webkit) {
      // Note that if we create the P with this invalid contentImage directly, Chrome cleans it up differently to other browsers so we don't
  ...
  const sHideSaveContentAndHiddenStateWhileSaving = (editor: Editor) => Log.step('TBA', 'hide save contentImage and hidden state while saving', Step.sync(() => {
    ...
    Assert.eq('hide save contentImage and hidden state while saving', '<p>xyz</p>', lastEvent.content);
    Assert.eq('hide save contentImage and hidden state while saving', '<p>xyz</p>', elm.value);
