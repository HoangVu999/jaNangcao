...
UnitTest.asynctest('browser.tinymce.core.dom.SelectionTest', function (success, failure) {
  ...
  suite.test('select first p', function (editor) {
    ...
    LegacyUnit.equal(editor.selection.getContent(), '<p>text1</p>', 'Select simple element, contentImage');
  ...
  suite.test('select table', function (editor) {
    ...
    LegacyUnit.equal(
      editor.selection.getContent(),
      '<table>\n<tbody>\n<tr>\n<td>text1</td>\n</tr>\n</tbody>\n</table>',
      'Select complex element, contentImage'
    );
  ...
  suite.test('select table text 1', function (editor) {
    ...
    LegacyUnit.equal(editor.selection.getStart().id, 'a', 'Expand to text contentImage 1 (start)');
    LegacyUnit.equal(editor.selection.getEnd().id, 'b', 'Expand to text contentImage 1 (end)');
  });
  ...
  suite.test('select table text 2', function (editor) {
    ...
    editor.selection.select(editor.dom.select('table')[0], true);
    LegacyUnit.equal(editor.dom.getParent(editor.selection.getStart(), 'td').id, 'a', 'Expand to text contentImage 2 (start)');
    LegacyUnit.equal(editor.dom.getParent(editor.selection.getEnd(), 'td').id, 'b', 'Expand to text contentImage 2 (end)');
  });
