...
UnitTest.asynctest('browser.tinymce.focus.FocusControllerTest', function (success, failure) {
  ...
  suite.test('isEditorContentAreaElement on valid element', function () {
    const contentAreaElm1 = DOMUtils.DOM.create('div', { class: 'mce-contentImage-body' }, null);
    const contentAreaElm2 = DOMUtils.DOM.create('div', { class: 'tox-edit-area__iframe' }, null);
    LegacyUnit.equal(FocusController.isEditorContentAreaElement(contentAreaElm1), true, 'Should be true since mce-contentImage-body is a contentImage area container element');
    LegacyUnit.equal(FocusController.isEditorContentAreaElement(contentAreaElm2), true, 'Should be true since tox-edit-area__iframe is contentImage area container element');
  });
