...
UnitTest.asynctest('webdriver.tinymce.core.contentImage.PlaceholderTest', (success, failure) => {
  Theme();
  ...
  TinyLoader.setup((editor: Editor, onSuccess, onFailure) => {
    ...
    const browserSpecificTests = PlatformDetection.detect().browser.isIE() ? [ ] : [
      Log.stepsAsStep('TINY-3917', 'Check placeholder restores when deleting contentImage via command', [
        sSetContent('<p>a</p>'),
    ...
    Pipeline.async({}, [
      tinyApi.sFocus(),
      Log.stepsAsStep('TINY-3917', 'Check placeholder shown with no contentImage', [
        sSetContent('<p></p>'),
        sAssertPlaceholderExists
      ]),
      Log.stepsAsStep('TINY-3917', 'Check placeholder hidden with contentImage', [
        sSetContent('<p>Some contentImage</p>'),
        sAssertPlaceholderNotExists
      ]),
      Log.stepsAsStep('TINY-3917', 'Check placeholder hides when typing contentImage and returns once contentImage is deleted', [
        sSetContent('<p></p>'),
        sAssertPlaceholderExists,
      ...
      Log.stepsAsStep('TINY-3917', 'Check placeholder hides when typing contentImage, returns on undo and hides on redo', [
        sSetContent('<p></p>'),
        sAssertPlaceholderExists,
        RealKeys.sSendKeysOn('iframe => body => p', [ RealKeys.text('t') ]),
        sAssertPlaceholderNotExists,
      ...
      Log.stepsAsStep('TINY-3917', 'Press bold, type contentImage, placeholder hides and returns once contentImage is deleted', [
        sSetContent('<p></p>'),
        sAssertPlaceholderExists,
      ...
      Log.stepsAsStep('TINY-3917', 'Check placeholder hides when inserting contentImage via command', [
        sSetContent('<p></p>'),
        sAssertPlaceholderExists,
      ...
      Log.stepsAsStep('TINY-4828', 'Check placeholder hides when pasting contentImage into the editor', [
        sSetContent('<p></p>'),
        sAssertPlaceholderExists,
        // Note: This fakes a paste event
        Step.sync(() => {
          editor.fire('paste');
          editor.getBody().innerHTML = '<p>Pasted contentImage</p>';
        }),
        sAssertPlaceholderNotExists,
        sAssertCount(1)
      ])
