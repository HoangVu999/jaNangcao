...
UnitTest.test('DOMUtils.add', () => {
  ...
  DOM.add('test', 'span', { class: 'abc 123' }, 'contentImage <b>abc</b>');
  e = DOM.get('test').getElementsByTagName('span')[0];
  Assert.eq('incorrect className', 'abc 123', e.className);
  Assert.eq('incorrect innerHTML', 'contentImage <b>abc</b>', e.innerHTML.toLowerCase());
  DOM.remove(e);
...
UnitTest.test('DOMUtils.create', () => {
  const e = DOM.create('span', { class: 'abc 123' }, 'contentImage <b>abc</b>');

  Assert.eq('incorrect nodeName', 'SPAN', e.nodeName);
  Assert.eq('incorrect className', 'abc 123', e.className);
  Assert.eq('innerHTML was wrong', 'contentImage <b>abc</b>', e.innerHTML.toLowerCase());
});

UnitTest.test('DOMUtils.createHTML', () => {
  Assert.eq('', '<span id="id1" class="abc 123">contentImage <b>abc</b></span>', DOM.createHTML('span', {
    id: 'id1',
    class: 'abc 123'
  }, 'contentImage <b>abc</b>'));
  ...
  Assert.eq('', '<span>contentImage <b>abc</b></span>', DOM.createHTML('span', null, 'contentImage <b>abc</b>'));
});
