...
UnitTest.asynctest('browser.tinymce.core.ClickContentEditableFalseTest', function (success, failure) {
  ...
  TinyLoader.setupLight(function (editor, onSuccess, onFailure) {
    ...
    Pipeline.async({}, [
      tinyApis.sFocus(),
      Logger.t('Click on contentImage editable false', GeneralSteps.sequence([
        tinyApis.sSetContent('<p contenteditable="false">a</p>'),
        sClickMiddleOf(editor, [ 1 ]),
        tinyApis.sAssertSelection([], 0, [], 1)
      ])),
      Logger.t('Click on contentImage editable false inside contentImage editable true', GeneralSteps.sequence([
      ...
      Logger.t('Click on contentImage editable true inside contentImage editable false', GeneralSteps.sequence([
        tinyApis.sSetContent('<div contenteditable="false"><p contenteditable="true">a</p></div>'),
        sClickMiddleOf(editor, [ 1, 0 ]),
        tinyApis.sAssertSelection([ 0, 0, 0 ], 1, [ 0, 0, 0 ], 1)
      ])),
      Logger.t('Click on contentImage editable false inside contentImage editable true and then on contentImage editable true and type', GeneralSteps.sequence([
      ...
      Logger.t('Click on contentImage editable false then outside on contentImage editable inherit', GeneralSteps.sequence([
