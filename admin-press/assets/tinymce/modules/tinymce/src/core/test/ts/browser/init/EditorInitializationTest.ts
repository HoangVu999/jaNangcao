...
UnitTest.asynctest('browser.tinymce.core.init.EditorInitializationTest', function (success, failure) {
  ...
  suite.asyncTest('target (initialised properly)', function (_, done) {
    ...
    EditorManager.init({
      ...
      content_css: '/project/tinymce/js/tinymce/skins/contentImage/default',
  ...
  const mAssertEditors = Step.label('mAssertEditors', Step.stateful(function (editors: any[], next, _die) {
    Assertions.assertHtml('Editor contents should be the first div contentImage', '<p>a</p>', editors[0].getContent());
    Assertions.assertHtml('Editor contents should be the second div contentImage', '<p>b</p>', editors[1].getContent());
    ...
    Assertions.assertEq(
      'Should only be two skin files the skin and the contentImage for inline mode',
      [ 'skin.min.css', 'contentImage.inline.min.css' ],
      getSkinCssFilenames()
    );
  ...
  const sInitAndAssertContent = (html: string, selector: string, expectedEditorContent: string) => Step.async((done) => {
    ...
    EditorManager.init({
      ...
      content_css: '/project/tinymce/js/tinymce/skins/contentImage/default',
      init_instance_callback(ed) {
        Assertions.assertEq('Expect editor to have contentImage', expectedEditorContent, ed.getContent({ format: 'text' }));
        teardown(done);
      }
    });
  });
  ...
  Pipeline.async({}, [
    ...
    Logger.t('Initialize on textarea with initial contentImage', GeneralSteps.sequence([
      sInitAndAssertContent('<textarea>Initial Content</textarea>', 'textarea', 'Initial Content')
    ])),
    Logger.t('Initialize on input with initial contentImage', GeneralSteps.sequence([
      sInitAndAssertContent('<input value="Initial Content">', 'input', 'Initial Content')
    ])),
    Logger.t('Initialize on list item with initial contentImage', GeneralSteps.sequence([
