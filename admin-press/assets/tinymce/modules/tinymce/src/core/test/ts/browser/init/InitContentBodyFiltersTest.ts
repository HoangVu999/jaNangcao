...
UnitTest.asynctest('browser.tinymce.core.init.InitContentBodyFiltersTest', (success, failure) => {
  ...
  TinyLoader.setupLight((editor, onSuccess, onFailure) => {
    ...
    Pipeline.async({}, [
      Log.stepsAsStep('TINY-4742', 'Insert contentImage to activate node filters, check contentImage is in editor', [
        tinyApis.sFocus(),
        tinyApis.sSetContent('<p style="color: rgb(255, 0, 0);">abc</p>'),
        // If an exception occurs, then no contentImage will be inserted into the editor
