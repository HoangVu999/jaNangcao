  ...
  TinyLoader.setupLight((editor: Editor, onSuccess, onFailure) => {
    ...
    Pipeline.async({}, [
      ...
      Logger.t('Should be text contents on a range selection', GeneralSteps.sequence([
        ...
        sAssertGetContent('Should be some contentImage', editor, 'a')
      ])),
      Logger.t('Should be text contents provided by override handler', GeneralSteps.sequence([
        tinyApis.sSetContent('<p>a</p>'),
        tinyApis.sSetSelection([ 0, 0 ], 0, [ 0, 0 ], 1),
        sAssertGetContentOverrideBeforeGetContent('Should be overridden contentImage', editor, 'X')
      ])),
      Logger.t(`Should be text contents when editor isn't focused and format is text`, GeneralSteps.sequence([
        ...
        sFocusDiv,
        sAssertGetContent('Should be some contentImage', editor, 'ab', { format: 'text' }),
        sRemoveTestDiv
      ])),
      Logger.t('Should be text contentImage with newline', GeneralSteps.sequence([
        tinyApis.sSetContent('<p>ab<br/>cd</p>'),
        tinyApis.sSetSelection([ 0, 0 ], 0, [ 0, 2 ], 2),
        sAssertGetContent('Should be some contentImage', editor, `ab${Env.ie === 11 ? '\r\n' : '\n'}cd`, { format: 'text' })
      ])),
      Logger.t('Should be text contentImage with leading visible spaces', GeneralSteps.sequence([
        tinyApis.sSetContent('<p>contentImage<em> Leading space</em></p>'),
        tinyApis.sSetSelection([ 0 ], 1, [ 0 ], 2),
        sAssertGetContent('Should be some contentImage', editor, ' Leading space', { format: 'text' })
      ])),
      Logger.t('Should be text contentImage with trailing visible spaces', GeneralSteps.sequence([
        tinyApis.sSetContent('<p><em>Trailing space </em>contentImage</p>'),
        tinyApis.sSetSelection([ 0 ], 0, [ 0 ], 1),
        sAssertGetContent('Should be some contentImage', editor, 'Trailing space ', { format: 'text' })
      ])),
      Logger.t('Should be text contentImage without non-visible leading/trailing spaces', GeneralSteps.sequence([
        ...
        browser.isIE() || browser.isEdge() ?
          sAssertGetContent('Should be some contentImage', editor, 'spaces ', { format: 'text' }) :
          sAssertGetContent('Should be some contentImage', editor, 'spaces', { format: 'text' }),
