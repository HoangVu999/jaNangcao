...
UnitTest.asynctest('browser.tinymce.core.contentImage.InsertContentCommandTest', (success, failure) => {
  ...
  suite.test('mceInsertContent - legacy contentImage', function (editor) {
    // Convert legacy contentImage
    editor.setContent('<p>1</p>');
    const rng = editor.dom.createRng();
    rng.setStart(editor.dom.select('p')[0].firstChild, 0);
    rng.setEnd(editor.dom.select('p')[0].firstChild, 1);
  ...
  suite.test('mceInsertContent - mixed inline contentImage inside td', function (editor) {
    // Forced root block
