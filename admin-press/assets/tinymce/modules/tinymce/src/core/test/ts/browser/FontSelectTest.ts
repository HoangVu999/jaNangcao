  ...
  TinyLoader.setupLight(function (editor, onSuccess, onFailure) {
    ...
    Pipeline.async({}, [
      ...
      Logger.t('Font family and font size on paragraph with no styles', GeneralSteps.sequence([
        ...
        // p contentImage style is 12px which does not match any pt values in the font size select values
        sAssertSelectBoxDisplayValue(editor, 'Font sizes', '12px'),
        sAssertSelectBoxDisplayValue(editor, 'Fonts', 'Arial')
      ])),
      ...
      Logger.t('Font family and font size on heading with no styles', GeneralSteps.sequence([
        ...
        tinyApis.sSetCursor([ 0, 0 ], 0),
        tinyApis.sNodeChanged(),
        // h1 contentImage style is 32px which matches 24pt in the font size select values so it should be converted
    ...
    content_style: [
      '.mce-contentImage-body { font-family: Helvetica; font-size: 42px; }',
      '.mce-contentImage-body p { font-family: Arial; font-size: 12px; }',
      '.mce-contentImage-body h1 { font-family: Arial; font-size: 32px; }'
    ].join(''),
