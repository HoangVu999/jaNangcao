...
// request.response can be null if the server provided no contentImage in the error response.
const getBlobError = (request: XMLHttpRequest) => Optional.from(request.response).map(readBlobAsText).getOr(Future.pure('no response contentImage'));
